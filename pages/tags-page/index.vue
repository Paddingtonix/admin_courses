<template>
	...
	<template v-for="tag in tagsData.tags" :key="tag.id">
		<TableRowCmp
			...
			:id="tag.id"
			:redirect="false"
			:name="checkForFlooding(tag.name, 20)"
			:status="checkForFlooding(tag.headingName, 15)"
			:authors="checkForFlooding(tag.localizations.ru, 40)"
			@click="
				openModalAddTag(
					{
	...
	>
		<PaginationCmp
			...
			"
			@change-page="changePageTags($event)"
			:pages_count="tagsStore.$state.numberOfPages"
			:current-page="tagsStore.$state.currentPage"
		/>
		<SelectorCmp
...
const changePageTags = (page: number) => {
	tagsStore.$patch({
		currentPage: page,
	});
	tagsStore.getTags({ text: tagsStore.searchValue });
};
