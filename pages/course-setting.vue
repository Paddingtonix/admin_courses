		...
		<div class="oil-page oil-course-setting">
			...
			<template v-if="active_tab === 1">
				<div class="oil-course-setting__settings">
					...
					>
						...
						<p
							class="oil-course-setting__settings__notification__text"
						>
							Обратите внимание, при частичной покупке ученик не
							сможет получить сертификат (участник сможет получить
							сертификат только при покупке всех глав курса)
						</p>
					</div>
					<div class="oil-course-setting__settings__table">
						...
						>
							...
							>
								...
								<div
									v-if="
										course_setting.value.CourseType !==
										'Asynchronous'
									"
									class="oil-course-setting__settings__table__column__cell"
								>
									...
									<span v-if="course_table[1].price">
										...
										>
											<span
												...
												>Суммарная стоимость всех глав,
												созданных во вкладке
												<a>Содержание</a></span
											>
										</div>
									</span>
							...
							>
								...
								>
									...
									>
										<span
											...
											>Стоимость курса формируется как
											сумма стоимости всех глав, созданных
											во вкладке <a>Содержание</a></span
										>
									</div>
								</div>
								<div
								...
								>
									<InputCmp
										...
										@set_value="
											updateFormData($event, input_idx)
										"
									/>
								</div>
								<div
								...
								>
									...
									>
										...
										<span v-if="show_error"
											>Курсу должно быть присвоено хотя бы
											одно направлние!</span
										>
									</div>
								</div>
							</div>
			...
			<template v-else-if="active_tab === 3">
				<div class="oil-course-setting__content">
					...
					>
						<i
							class="oil-course-setting__content__info__attention__icon"
						>
							<svg
								xmlns="http://www.w3.org/2000/svg"
								width="40"
						...
						<p
							class="oil-course-setting__content__info__attention__text"
						>
							Курс архивирован, вы не можете просмотреть его
							наполнение на сайте. Доступна опция выгрузки курса
							на ПК в формате PDF.
						</p>
						<BtnCmp
							:text="'Скачать PDF'"
					...
					>
						...
						>
							<template v-if="content_inner.value.initialPage">
								...
								<CourseArchitectureIcons
									...
									:arrow="false"
									:delete_id="
										content_inner.value.initialPage.id
									"
									:delete_type="'Page'"
							...
							<transition name="fade">
								<CourseArchitectureAddBlock
									...
									:block_id="
										Number($route.query.course) || undefined
									"
						...
						>
							<template v-if="content_inner.value.initialTesting">
								...
								<CourseArchitectureIcons
									...
									:arrow="false"
									:delete_id="
										content_inner.value.initialTesting.id
									"
									:delete_type="'Testing'"
									@delete-trigger="reloadContent"
								/>
						...
						<template
							v-for="(
								part, idx
							) in content_inner.value.parts.sort(
							...
							>
								...
								<CourseArchitectureIcons
									...
									:arrow="{
										...
										down:
											idx ===
											content_inner.value.parts.length - 1
												? false
												: true,
									}"
								/>
								<transition name="fade">
									<CourseArchitectureAddBlock
										...
										v-if="
											idx ===
												content_inner.value.parts
													.length -
													1 &&
											storeStateCourse.status !==
												'Archived'
										"
									/>
								</transition>
						...
						>
							<template v-if="content_inner.value.finalTesting">
								...
								<CourseArchitectureIcons
									...
									"
									:delete_id="
										content_inner.value.finalTesting.id
									"
									:delete_type="'Testing'"
									@delete-trigger="reloadContent"
									:arrow="false"
	...
	setup() {
			...
			if (state) {
				...
				axios
					...
					.finally(() => {
						...
						axios
							.get(
								`/admin/v1/Course/${route.query.search}/content`
							)
							.then((struct_response) => {
								content_inner.value = struct_response.data;
								reload_state.value = false;
		...
		const saveSettings = () => {
			if (picked_directions_filtered.value.length === 0) {
				...
				axios
					.patch(
						`/admin/v1/Course/${route.query.search}/settings`,
						formData
					)
					.then((response) => {
						storeEditCourseSetting.canselEdit();
					...
					.catch((error) => {
						console.error(
							"Ошибка при сохранении настроек курса:",
							error
						);
					})
					.finally(() => {
				...
				axios
					...
					.then((info_course) => {
						...
						(course_table[1].authors =
							info_course.data.AuthorEmails),
							(course_table[1].price = info_course.data
								.PriceInRubles
								? String(
										info_course.data.PriceInRubles
								  ).replace(/\B(?=(\d{3})+(?!\d))/g, " ")
						...
						course_table[1].duration = info_course.data
							...
							: info_course.data.DurationAcademicHours;
						course_table[1].workload = info_course.data
							.DurationWorkDays
