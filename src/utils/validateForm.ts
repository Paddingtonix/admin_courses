...
const hasChanges = (
  ...
  export const validateForm: TValidateForm = ({
	...
	const changesValid = options.checkChanges ? hasChanges(currentForm, initialForm) : true;
  
	// Если включена проверка обязательных полей
	const fieldsFilledValid = options.checkRequiredFields ? allFieldsFilled(currentForm, requiredFields) : true;
