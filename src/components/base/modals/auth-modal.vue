<template>
    <div :class="`oil-modal__authorization`">
        <span :class="`oil-modal__authorization__title`"
            >Вход в Admin Course</span
        >
        <div
            :class="`oil-modal__authorization__form`"
            v-for="form_item in form_authorization"
            :key="form_item.id"
        >
            <InputCmp
                :type="form_item.type"
            ...
            />
            <svg
                :class="`oil-modal__authorization__icon`"
                v-if="form_item.type === 'password'"
                @click="visiblePassword(form_item.type)"
            ...
            >
                <path
                    :class="{ error: form_item.error.length }"
        ...
        />
        <BtnCmp :text="'Войти'" @click="loginSite" />
    </div>
</template>
<script lang="ts">
...
export default defineComponent({
    ...
    setup() {
        ...
        const validCheck = (form_item: Form) => {
            ...
            if (form_item.value) {
                if (
                    form_item.id === "username" &&
                    !form_item.value.match(/@/gi)
                ) {
                    form_item.error =
                        "Неверный формат данных. Введите корректный email";
                } else if (!regExp.test(form_item.value)) {
                    form_item.error =
                        "Неверный формат данных. Введите корректный email";
                } else if (
                    form_authorization.every((value) => value.value.length)
                ) {
                    form_item.error = "";
                } else {
                    form_item.error = "";
                ...
                form_item.error =
                    "Это поле обязательно к заполнению для авторизации";
            }
        };
