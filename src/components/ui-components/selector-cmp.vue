<template>
    <div class="oil-selector" @click="openList" :class="{ '_error-frame': error }">
        <div class="oil-selector__chooses">
            <label v-if="!choosesVariable">{{ label }}</label>
            <span>{{ choosesVariable }}</span>
            <svg :class="{'_active-list': listOpened}" class="oil-selector__chooses__chevron" xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
...
export default defineComponent({
    ...
    setup(props, { emit }) {
        ...
        const selectValue = (val: string) => {
            ...
            emit('setValue', { value: choosesVariable.value, type: props.type });
        };
